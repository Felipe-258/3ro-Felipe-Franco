...
use App\Http\Controllers\LogController; // Agregar la importación
...
class StudentController extends Controller
{
    ...
    public function store(StoreStudentRequest $request): RedirectResponse
    {
        ...
        if ($age >= 18) {
            ...
            Student::create($data);
            $accion = 'alta';
            $logController->logCRUD($request, auth()->user()->id, $accion);
     ...
     */
    public function destroy(Student $student, Request $request): RedirectResponse
    {
        $logController = new LogController();
        $accion = 'baja';
        $logController->logCRUD($request, auth()->user()->id, $accion);
        // Eliminar registros relacionados en la tabla `assists`
        DB::table('assists')->where('student_id', $student->id)->delete();
        DB::table('notes')->where('student_id', $student->id)->delete();
        // Eliminar el estudiante usando DB
        DB::table('students')->where('id', $student->id)->delete();
        // Aquí deberías llamar al método correcto del controlador LogController
